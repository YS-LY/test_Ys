...
spark = SparkSession.builder \
    .appName("create_ads_product_macro_monitor_split") \
...
# --------------------------
# 表1：商品访问基础指标表
# 存储：访问核心指标（访客数、浏览量、停留时长、跳出率）
# --------------------------
create_visit_table_sql = """
CREATE TABLE IF NOT EXISTS gd3.ads_product_visit_stats (
...
STORED AS PARQUET
LOCATION 'hdfs://cdh01:8020/bigdata_warehouse/gd3/ads_product_visit_stats'
TBLPROPERTIES (
    'parquet.compression' = 'snappy',
    'comment' = '商品访问基础指标表，存储访问核心指标'
);
"""
spark.sql(create_visit_table_sql)
print("ads_product_visit_stats表结构创建完成")
...
CREATE TABLE IF NOT EXISTS gd3.ads_product_interaction_stats (
    statistical_time STRING COMMENT '统计时间',
    product_id STRING COMMENT '商品ID',
    product_name STRING COMMENT '商品名称',
    category_id STRING COMMENT '叶子类目ID',
...
STORED AS PARQUET
LOCATION 'hdfs://cdh01:8020/bigdata_warehouse/gd3/ads_product_interaction_stats'
TBLPROPERTIES (
    'parquet.compression' = 'snappy',
    'comment' = '商品互动行为指标表，存储收藏、加购等互动指标'
);
"""
spark.sql(create_interaction_table_sql)
print("ads_product_interaction_stats表结构创建完成")
...
CREATE TABLE IF NOT EXISTS gd3.ads_product_transaction_stats (
    statistical_time STRING COMMENT '统计时间',
    product_id STRING COMMENT '商品ID',
    product_name STRING COMMENT '商品名称',
    category_id STRING COMMENT '叶子类目ID',
...
STORED AS PARQUET
LOCATION 'hdfs://cdh01:8020/bigdata_warehouse/gd3/ads_product_transaction_stats'
TBLPROPERTIES (
    'parquet.compression' = 'snappy',
    'comment' = '商品交易转化指标表，存储下单、支付等交易指标'
);
"""
spark.sql(create_transaction_table_sql)
print("ads_product_transaction_stats表结构创建完成")
...
# 插入访问基础指标表
insert_visit_sql = f"""
INSERT INTO gd3.ads_product_visit_stats
SELECT
...
"""
spark.sql(insert_visit_sql)
print(f"ads_product_visit_stats表 {bizdate} 日维度数据插入完成")

# 插入互动行为指标表
insert_interaction_sql = f"""
INSERT INTO gd3.ads_product_interaction_stats
SELECT
    a.date AS statistical_time,
    a.product_id,
    b.product_name,
...
WHERE a.date = '{bizdate}';
"""
spark.sql(insert_interaction_sql)
print(f"ads_product_interaction_stats表 {bizdate} 日维度数据插入完成")

# 插入交易转化指标表
insert_transaction_sql = f"""
INSERT INTO gd3.ads_product_transaction_stats
SELECT
    a.date AS statistical_time,
    a.product_id,
    b.product_name,
...
WHERE a.date = '{bizdate}';
"""
spark.sql(insert_transaction_sql)
print(f"ads_product_transaction_stats表 {bizdate} 日维度数据插入完成")
