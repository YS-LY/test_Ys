...
def process_ads_data():
    ...
    try:
        ...
        weight = {
            "click_rate": 0.35,       # 流量获取权重
            "conversion_rate": 0.30,  # 流量转化权重
            "content_interact_rate": 0.1,  # 内容营销权重
            "new_user_rate": 0.1,    # 客户拉新权重
            "service_rating": 0.15    # 服务质量权重
        }
        ...
        score_df = base_df.withColumn(
            ...
            round(
                ...
                (col("service_rating") * weight["service_rating"] * 100),
                2
            )
        ).withColumn(
