...
def process_order_cargo_type_stats(spark):
    # 配置参数
    db_name, target_table = "tms", "ads_order_cargo_type_stats"
    table_path = "hdfs://cdh01:8020/warehouse/tms/ads/ads_order_cargo_type_stats"
    current_dt = "20250712"
    ...
    try:
        ...
        existing_df = spark.createDataFrame([], StructType([
            ...
            StructField("cargo_type_name", StringType(), True),
    ...
    order_nd = safe_read("dws_trade_org_cargo_type_order_nd", StructType([
        ...
        .select("dt", "recent_days", "cargo_type", "cargo_type_name", "order_count", "order_amount")

    # 合并历史数据与新数据
    final_df = existing_df.unionByName(order_1d).unionByName(order_nd)
...
if __name__ == "__main__":
    ...
    try:
        process_order_cargo_type_stats(spark)
    finally:
        spark.stop()
