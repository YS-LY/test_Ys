...
def process_shift_stats(spark):
    # 配置参数
    db_name, target_table = "tms", "ads_shift_stats"
    table_path = "hdfs://cdh01:8020/warehouse/tms/ads/ads_shift_stats"
    current_dt = "20250712"
    ...
    # 读取源数据并处理（对应SQL中的union部分）
    shift_nd = safe_read("dws_trans_shift_trans_finish_nd", StructType([
        StructField("dt", StringType(), True),
        StructField("recent_days", ByteType(), True),
        StructField("shift_id", LongType(), True),
        StructField("trans_finish_count", LongType(), True),
        ...
        .withColumn("dt", F.lit(current_dt))  \
        .select("dt", "recent_days", "shift_id",
            "trans_finish_count", "trans_finish_distance",
            "trans_finish_dur_sec", "trans_finish_order_count")



    final_df = existing_df.unionByName(shift_nd)
...
# 重建目标表（对应SQL中的DROP TABLE和CREATE TABLE）
