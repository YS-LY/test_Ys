...
def process_trans_stats(spark):
    # 配置参数
    db_name, target_table = "tms", "ads_trans_stats"
    table_path = "hdfs://cdh01:8020/warehouse/tms/ads/ads_trans_stats"
    current_dt = "20250712"
    ...
    # 读取源数据
    trans_1d = safe_read("dws_trans_org_truck_model_type_trans_finish_1d", StructType([
        StructField("dt", StringType(), True),
        StructField("trans_finish_count", LongType(), True),
        StructField("trans_finish_distance", DecimalType(16, 2), True),
        StructField("trans_finish_dur_sec", LongType(), True)
    ...
    trans_nd = safe_read("dws_trans_shift_trans_finish_nd", StructType([
        StructField("dt", StringType(), True),
        StructField("recent_days", ByteType(), True),
        StructField("trans_finish_count", LongType(), True),
        StructField("trans_finish_distance", DecimalType(16, 2), True),
    ...
    recent_nd = trans_nd.groupBy("recent_days").agg(
        F.lit(current_dt).alias("dt"),
        F.sum("trans_finish_count").alias("trans_finish_count"),
        F.sum("trans_finish_distance").alias("trans_finish_distance"),
        F.sum("trans_finish_dur_sec").alias("trans_finish_dur_sec")
    ).select("dt", "recent_days", "trans_finish_count", "trans_finish_distance", "trans_finish_dur_sec")
